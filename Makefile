...
fixlint: ## fix lint issues using autoflake, autopep8, and isort
	find semsch -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive semsch
	isort --apply --atomic --recursive semsch
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source semsch -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

clean-docs: ## remove previously built docs
	rm -f docs/semsch.rst
	rm -f docs/modules.rst
	$(MAKE) -C docs clean

docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc -o docs/ semsch
	$(MAKE) -C docs html
